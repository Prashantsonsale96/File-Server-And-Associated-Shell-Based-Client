...
static char server[MAX_NO_TOKENS], serverport[MAX_NO_TOKENS];
...
struct cmdline* creatpt(char** tokens, int start, int end, int* validcmd)
{
	...
	if(!strcmp(tokens[0],commands[0]) || !strcmp(tokens[0],commands[1]) || !strcmp(tokens[0],commands[2]) || !strcmp(tokens[0],commands[3]) || !strcmp(tokens[0],commands[4]))
		found = 1;
	if(found)
	{
		*validcmd = 1;
...
/*
 Function just copy Server-IP and Server-port into global variable.
*/
void Server_Implementation(char** tokens, int NO_tokens)
{
	if(NO_tokens != 3)
	{
		fprintf(stderr,"Usage: Server ServerIP Server-port\n");
		return;
	}
	bzero(server,MAX_NO_TOKENS);
	bzero(serverport,MAX_NO_TOKENS);
...
void run_command(struct cmdline* cmd)
{
	...
	else if(cmd->type == BASECMD)
	{
		...
		{
			CD_Implementation(globltokens,tokenscount);
		}
		
		//command server
		else if(!strcmp(tokens[0],commands[1]))
		{
			Server_Implementation(globltokens,tokenscount);
		}
		...
		else if(!strcmp(tokens[0],commands[2]))
		{
			...
			if(server[0] == 0 || serverport[0] == 0) {
				fprintf(stderr,"Server not configured. Use server command to configure server\n");
				cleanup_(globltokens,globlcmd);
				exit(1);
			}
			execl("./get-file", "./get-file", tokens[1], server, serverport, "display", (char*) NULL);
			cleanup_(globltokens,globlcmd);
		...
		else if(!strcmp(tokens[0],commands[3]))
		{
			if(tokenscount < 2)
			{
				fprintf(stderr,"Usage: getfl filename-1 filename-2 ..... filename-n");
				cleanup_(globltokens,globlcmd);
				exit(1);
			}
			...
			for(i=1;i<tokenscount;++i)
			{
				...
				if(ret == 0)
				{
					execl("./get-file", "./get-file", tokens[i], server, serverport, "nodisplay", (char*) NULL);
					cleanup_(globltokens,globlcmd);	
					exit(1);	// in case execl fails
				}
				waitpid(ret, &status, 0);				// wait before forking of the next operation.(Hence the seq implementation).
			}
			cleanup_(globltokens,globlcmd);
			exit(0);
		}
		...
		else if(!strcmp(tokens[0],commands[4]))
		{
			if(tokenscount < 2)
			{
				fprintf(stderr,"Usage: getpl filename-1 filename-2...... filename-n");
				cleanup_(globltokens,globlcmd);
				exit(1);
			}
			...
			if(server[0] == 0 || serverport[0] == 0)
			{
				fprintf(stderr,"Server not configured. Use server command to configure\n");
				cleanup_(globltokens,globlcmd);
				exit(1);
			}

			int i,status;
			int pid[tokenscount-1];
			for(i=1;i<tokenscount;++i)
			{
				int ret = fork();
				if(ret == 0)
				{
					execl("./get-file","./get-file", tokens[i], server, serverport, "nodisplay",(char*)NULL);
					cleanup_(globltokens,globlcmd);         //In case exec fails.
					exit(1);
				}
				pid[i-1] = ret;
			}
...
void main()
{
	...
	while(1)
	{
		...
		if(globlcmd->type == BASECMD)
		{
			...
			if(!strcmp(globltokens[0],commands[1]))
			{
				Server_Implementation(globltokens,NO_tokens);
				cleanup_(globltokens,globlcmd);
				continue;
			}
		}
